...
static void handle_zoom(void) {
  ...
  if (mouse_wheel_delta != 0 && !g_state->is_drawing && !IsKeyDown(KEY_LEFT_CONTROL) && !IsKeyDown(KEY_RIGHT_CONTROL)) {
    ...
    Vector2 world     = {
      ...
      (mouse_pos.y - g_state->pan_target.y) / prev_zoom,
    };

    g_state->zoom_target =
        Clamp(g_state->zoom_target + mouse_wheel_delta * g_configuration->zoom_step, g_configuration->zoom_min, g_configuration->zoom_max);
    g_state->pan_target.x = mouse_pos.x - world.x * g_state->zoom_target;
    g_state->pan_target.y = mouse_pos.y - world.y * g_state->zoom_target;
  }
}
...
static void handle_flashlight(void) {
  ...
  if (g_state->flashlight_enabled && mouse_wheel_delta != 0) {
    if (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL)) {
      g_state->flashlight_radius_target -= mouse_wheel_delta * g_configuration->flashlight_radius_step;
      g_state->flashlight_radius_target =
